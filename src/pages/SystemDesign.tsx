...
export default function SystemDesign() {
  ...
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        const items = getContentByCategory('system-design');
        
        if (!topic && items.length > 0) {
          // Redirect to first topic if no topic specified
          navigate(`/system-design/${items[0].metadata.topic}`, { replace: true });
          return;
        }
  ...
  const systemDesignItems = getContentByCategory('system-design');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/system-design" className="text-blue-600 hover:underline">
            Go back to System Design topics
          </Link>
        </div>
      </div>
    );
  ...
  const currentIndex = systemDesignItems.findIndex((item) => item.metadata.topic === topic);
  const nextItem = currentIndex >= 0 && currentIndex < systemDesignItems.length - 1 ? systemDesignItems[currentIndex + 1] : null;
  const prevItem = currentIndex > 0 ? systemDesignItems[currentIndex - 1] : null;

  return (
    <div className="min-h-screen bg-white dark:bg-[#0d1117] transition-colors">
      <div className="flex">
        <Sidebar items={sidebarItems} title="System Design" />
          ...
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-10 py-8 lg:py-12">
            ...
            <div className="mt-16 pt-8 border-t border-gray-200 dark:border-[#30363d]">
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                {prevItem ? (
                  <Link
                    to={`/system-design/${prevItem.metadata.topic}`}
                ...
                {nextItem ? (
                  <Link
                    to={`/system-design/${nextItem.metadata.topic}`}
                    onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
