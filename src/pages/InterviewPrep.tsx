...
export default function InterviewPrep() {
  ...
  const currentIndex = interviewItems.findIndex((item) => item.metadata.topic === topic);
  const nextItem = currentIndex >= 0 && currentIndex < interviewItems.length - 1 ? interviewItems[currentIndex + 1] : null;
  const prevItem = currentIndex > 0 ? interviewItems[currentIndex - 1] : null;
          ...
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-10 py-8 lg:py-12">
            ...
            <div className="mt-16 pt-8 border-t border-gray-200 dark:border-[#30363d]">
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                {prevItem ? (
                  <Link
                    to={`/interview/${prevItem.metadata.topic}`}
                ...
                {nextItem ? (
                  <Link
                    to={`/interview/${nextItem.metadata.topic}`}
                    onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
